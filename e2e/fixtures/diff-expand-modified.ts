/**
 * Modified version — large file for expandable collapsed region e2e tests.
 *
 * Changes from base:
 * - Section 1: renamed APP_VERSION, added ENV constant, rewrote createLogger
 * - Section 3: added groupBy, changed range signature, removed zip
 * - Section 5: added validateEmail, changed validateRange to return warnings
 * - Section 6: updated exports to match
 *
 * Sections 2 (string utils, ~lines 28-87) and 4 (math utils, ~lines 125-195)
 * are completely unchanged, creating large collapsed regions (~55 lines each).
 */
...
const createLogger = (level: LogLevel, context?: string) => {
  const prefix = context ? `[${APP_NAME}:${context}]` : `[${APP_NAME}]`;
  return {
    level,
    prefix,
    log: (msg: string) => console.log(`${prefix} [${level}] ${msg}`),
    error: (msg: string) => console.error(`${prefix} [${level}] ${msg}`),
  };
};
...
const validateEmail = (val: string): ValidationResult => {
...
const validateRange = (val: number, min: number, max: number): ValidationResult => {
  ...
  if (val === min || val === max) warnings.push('Value is at boundary');
